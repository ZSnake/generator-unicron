...
gulp.task('serve-dev', function(callback: () => any) {
    var stream = runSequence('build-dev', 'inject', 'test-for-build', 'server_start', callback);
    return stream;
}
    );
...
gulp.task('lintHtmls', function () {
    ...
    return gulp
        .src(config.sourceHtmls)
        .pipe(plugins.cached(cache))
        .pipe(plugins.htmlhint(config.htmlHintOptions))
...
gulp.task( 'server_start', function() {
    server.listen( { path: './server_test.js' } );
});
